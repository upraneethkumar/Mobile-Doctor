...
function DesktopStickyHeader({ theme, fonts, dark, isDesktop, onToggleDark }) {
  ...
  return (
    <header className="md-desktop-header" style={{
      borderBottom: `0.5px solid ${theme.border}`,
      background: `${theme.bg}e6`,
    }}>
      <div style={{
        maxWidth: 1360, margin: '0 auto', padding: `0 ${hPad}px`,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            ...
            <div style={{
              width: 28, height: 28, borderRadius: '50%', background: theme.bg,
              display: 'grid', placeItems: 'center',
              fontFamily: fonts.display, fontStyle: 'italic', fontSize: 17, color: theme.gold,
            }}>M</div>
          </div>
          <div>
            <div style={{ fontFamily: fonts.display, fontSize: 17, color: theme.fg, lineHeight: 1 }}>Mobile Doctor</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8.5, color: theme.fgMuted, letterSpacing: '0.2em', textTransform: 'uppercase', marginTop: 2 }}>Hyderabad · Est. 2013</div>
          </div>
        </div>

        {/* Nav links — desktop only */}
        {isDesktop && (
          <nav style={{ display: 'flex', gap: 32, fontFamily: fonts.body, fontSize: 13, color: theme.fgMuted }}>
            {[['Shop', '#sec-sales'], ['Services', '#sec-services'], ['Trust', '#sec-why'], ['Visit', '#sec-location'], ['Contact', '#sec-contact']].map(([label, href]) => (
              <a key={label} href={href} style={{ color: theme.fgMuted, textDecoration: 'none', letterSpacing: '0.04em' }}>{label}</a>
            ))}
          </nav>
        )}
...
function DesktopHero({ theme, fonts, intensity, isTablet }) {
  ...
  return (
    <div id="sec-hero" style={{
      padding: `90px ${hPad}px 90px`, position: 'relative', overflow: 'hidden',
      background: theme.bgGrad,
    }}>
      <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1.1fr 0.9fr', gap: 60, alignItems: 'center', position: 'relative', zIndex: 2 }}>
        <div>
        ...
        <div style={{
          position: 'relative', borderRadius: 24, overflow: 'hidden', height: isTablet ? 380 : 540,
...
function DesktopSales({ theme, fonts, cardKind, isTablet }) {
  ...
  return (
    <div id="sec-sales" className="md-section-anchor" style={{ padding: `${isTablet ? 70 : 90}px ${hPad}px`, borderTop: `0.5px solid ${theme.border}` }}>
      <div style={{ display: isTablet ? 'block' : 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 50 }}>
        <div>
          ...
          <h2 style={{ fontFamily: fonts.display, fontSize: isTablet ? 48 : 64, color: theme.fg, margin: '14px 0 0', letterSpacing: '-0.02em', lineHeight: 1 }}>The <span style={{ fontStyle: 'italic', color: theme.gold }}>showroom</span>.</h2>
        </div>
        <div style={{ fontFamily: fonts.body, fontSize: 13, color: theme.fgMuted, maxWidth: 380, lineHeight: 1.6, marginTop: isTablet ? 20 : 0 }}>
          A curated catalogue of mobiles and accessories — sourced from authorised distributors, photographed under daylight.
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${isTablet ? 2 : 3}, 1fr)`, gap: 18 }}>
        {products.map(p => (
          <div key={p.n} style={{ ...window.MD.cardStyle(cardKind, theme), borderRadius: 22, overflow: 'hidden', cursor: 'pointer' }}>
...
function DesktopServices({ theme, fonts, cardKind, isTablet }) {
  ...
  return (
    <div id="sec-services" className="md-section-anchor" style={{ padding: `${isTablet ? 70 : 90}px ${hPad}px`, borderTop: `0.5px solid ${theme.border}`, background: theme.glass }}>
      <div style={{ textAlign: 'center', marginBottom: 60 }}>
        ...
        <h2 style={{ fontFamily: fonts.display, fontSize: isTablet ? 48 : 64, color: theme.fg, margin: '14px 0 0', letterSpacing: '-0.02em', lineHeight: 1 }}>Quietly <span style={{ fontStyle: 'italic', color: theme.gold }}>obsessive</span> repair work.</h2>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${isTablet ? 2 : 4}, 1fr)`, gap: 18 }}>
        {services.map((s, i) => (
          <div key={s.n} style={{ ...window.MD.cardStyle(cardKind, theme), borderRadius: 22, padding: 30 }}>
            <div style={{
...
function DesktopWhy({ theme, fonts, cardKind, isTablet }) {
  ...
  return (
    <div id="sec-why" className="md-section-anchor" style={{
      padding: `${isTablet ? 70 : 90}px ${hPad}px`,
      borderTop: `0.5px solid ${theme.border}`,
      background: theme.bgGrad,
    }}>
      <div style={{ marginBottom: 60 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.32em', textTransform: 'uppercase' }}>04 — Why us</div>
        <h2 style={{ fontFamily: fonts.display, fontSize: isTablet ? 48 : 64, color: theme.fg, margin: '14px 0 0', letterSpacing: '-0.02em', lineHeight: 1 }}>
          The Hyderabad standard for <span style={{ fontStyle: 'italic', color: theme.gold }}>repair</span>.
        </h2>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1fr 1fr', gap: isTablet ? 40 : 60, alignItems: 'start' }}>
        {/* Quote card */}
        <div style={{ ...window.MD.cardStyle(cardKind, theme), borderRadius: 28, padding: '36px 36px 32px', position: 'relative', overflow: 'hidden' }}>
          <div style={{
            position: 'absolute', top: 8, right: 20,
            fontFamily: fonts.display, fontStyle: 'italic',
            fontSize: 130, color: theme.gold, opacity: 0.12, lineHeight: 1, pointerEvents: 'none',
          }}>"</div>
          <p style={{ fontFamily: fonts.display, fontSize: 22, lineHeight: 1.45, color: theme.fg, fontStyle: 'italic', margin: 0, letterSpacing: '-0.01em', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ marginTop: 28, display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{
              ...
              display: 'grid', placeItems: 'center',
              fontFamily: fonts.display, fontStyle: 'italic', fontSize: 20,
            }}>M</div>
            <div>
              <div style={{ fontFamily: fonts.body, fontSize: 14, color: theme.fg, fontWeight: 500 }}>The Mobile Doctor Promise</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: theme.fgMuted, letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: 3 }}>Hyderabad · Since 2013</div>
            </div>
          </div>
        </div>
        ...
        <div>
          {points.map((p, i) => (
            <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '20px 0', borderBottom: `0.5px solid ${theme.border}` }}>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.2em', width: 30, flexShrink: 0 }}>0{i + 1}</div>
              <div style={{ flex: 1, fontFamily: fonts.body, fontSize: 15, color: theme.fg, lineHeight: 1.4 }}>{p}</div>
...
function DesktopBrands({ theme, fonts, isTablet }) {
  ...
  return (
    <div style={{ padding: `${isTablet ? 70 : 90}px 0`, borderTop: `0.5px solid ${theme.border}`, overflow: 'hidden' }}>
      <div style={{ textAlign: 'center', marginBottom: 50 }}>
        ...
        <h2 style={{ fontFamily: fonts.display, fontSize: isTablet ? 40 : 56, color: theme.fg, margin: '14px 0 0', letterSpacing: '-0.02em', lineHeight: 1 }}>Every brand, <span style={{ fontStyle: 'italic', color: theme.gold }}>under one roof</span>.</h2>
      </div>
...
function DesktopLocation({ theme, fonts, cardKind, isTablet }) {
  ...
  return (
    <div id="sec-location" className="md-section-anchor" style={{
      padding: `${isTablet ? 70 : 90}px ${hPad}px`,
      borderTop: `0.5px solid ${theme.border}`,
    }}>
      <div style={{ marginBottom: 60 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.32em', textTransform: 'uppercase' }}>06 — Location</div>
        <h2 style={{ fontFamily: fonts.display, fontSize: isTablet ? 48 : 64, color: theme.fg, margin: '14px 0 0', letterSpacing: '-0.02em', lineHeight: 1 }}>
          Find the <span style={{ fontStyle: 'italic', color: theme.gold }}>atelier</span>.
        </h2>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1.1fr 0.9fr', gap: isTablet ? 40 : 60, alignItems: 'start' }}>
        {/* Map panel */}
        <div style={{ ...window.MD.cardStyle(cardKind, theme), borderRadius: 28, overflow: 'hidden' }}>
          <div style={{
            ...
            <div style={{ position: 'absolute', left: '50%', top: '48%', transform: 'translate(-50%, -50%)' }}>
              <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 100, height: 100, borderRadius: '50%', border: `1px solid ${theme.gold}`, animation: 'md-pulse-ring 2.4s ease-out infinite' }} />
              <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 100, height: 100, borderRadius: '50%', border: `1px solid ${theme.gold}`, animation: 'md-pulse-ring 2.4s ease-out 1.2s infinite' }} />
              <div style={{
                width: 46, height: 46, borderRadius: '50%', background: theme.gold,
                color: theme.navy, display: 'grid', placeItems: 'center',
                boxShadow: `0 10px 24px -4px ${theme.gold}`, position: 'relative', zIndex: 2,
              }}>
            ...
            {/* Corner coords */}
            <div style={{ position: 'absolute', top: 16, left: 20, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.2em' }}>17.5544° N</div>
            <div style={{ position: 'absolute', top: 16, right: 20, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.2em' }}>78.4905° E</div>
            <div style={{ position: 'absolute', bottom: 16, left: 20, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.fgMuted, letterSpacing: '0.2em' }}>HYD · KOMPALLY</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 32 }}>
          <div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.22em', textTransform: 'uppercase' }}>VISIT THE STUDIO</div>
            <div style={{ fontFamily: fonts.display, fontSize: 34, color: theme.fg, lineHeight: 1.15, margin: '12px 0 0', letterSpacing: '-0.01em' }}>Under Kompally Bridge</div>
            <div style={{ fontFamily: fonts.body, fontSize: 15, color: theme.fgMuted, lineHeight: 1.65, marginTop: 10 }}>
              via Gundlapochampally Village,<br />Hyderabad, Telangana
            </div>
          </div>

          {/* Hours card */}
          <div style={{ ...window.MD.cardStyle(cardKind, theme), borderRadius: 18, padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 14 }}>
            <AI name="clock" size={22} color={theme.gold} stroke={1.4} />
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: fonts.body, fontSize: 14, color: theme.fg }}>Open today · 10:00 — 21:00</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: theme.fgMuted, letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: 4 }}>Mon — Sun · Walk-ins welcome</div>
            </div>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#67e09b', boxShadow: '0 0 14px #67e09b', flexShrink: 0 }} />
          </div>

          {/* CTA buttons */}
          <div style={{ display: 'flex', gap: 14 }}>
            <button style={{
              flex: 1, height: 52, border: `0.5px solid ${theme.borderStrong}`,
              background: 'transparent', color: theme.fg, borderRadius: 14,
              fontFamily: fonts.body, fontWeight: 500, fontSize: 13, letterSpacing: '0.1em', textTransform: 'uppercase',
            ...
            <button style={{
              ...
              background: `linear-gradient(180deg, ${theme.gold}, ${theme.goldDeep})`,
              color: theme.navy, borderRadius: 14,
              fontFamily: fonts.body, fontWeight: 600, fontSize: 13, letterSpacing: '0.1em', textTransform: 'uppercase',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer',
...
function DesktopContact({ theme, fonts, cardKind, isTablet }) {
  ...
  return (
    <div id="sec-contact" className="md-section-anchor" style={{
      padding: `${isTablet ? 70 : 90}px ${hPad}px`,
      borderTop: `0.5px solid ${theme.border}`,
      background: theme.glass,
    }}>
      <div style={{ textAlign: 'center', marginBottom: 60 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.gold, letterSpacing: '0.32em', textTransform: 'uppercase' }}>07 — Contact</div>
        <h2 style={{ fontFamily: fonts.display, fontSize: isTablet ? 48 : 64, color: theme.fg, margin: '14px 0 0', letterSpacing: '-0.02em', lineHeight: 1 }}>
          Speak to the <span style={{ fontStyle: 'italic', color: theme.gold }}>Doctor</span>.
        </h2>
      </div>

      {/* Contact cards */}
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : 'repeat(3, 1fr)', gap: 18, marginBottom: 28 }}>
        {contacts.map(c => (
          <div key={c.l} style={{ ...window.MD.cardStyle(cardKind, theme), borderRadius: 22, padding: '32px 28px' }}>
            <div style={{
              width: 56, height: 56, borderRadius: 16,
              background: `linear-gradient(145deg, ${theme.gold}33, transparent)`,
              border: `0.5px solid ${theme.borderStrong}`, color: theme.gold,
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: theme.fgMuted, letterSpacing: '0.22em', textTransform: 'uppercase' }}>{c.l}</div>
            <div style={{ fontFamily: fonts.body, fontSize: 18, color: theme.fg, marginTop: 8, fontWeight: 500, lineHeight: 1.25 }}>{c.v}</div>
            {c.v2 && <div style={{ fontFamily: fonts.body, fontSize: 15, color: theme.fgMuted, marginTop: 6 }}>{c.v2}</div>}
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        borderRadius: 28, color: theme.navy, position: 'relative', overflow: 'hidden',
        display: 'flex', alignItems: 'center',
        justifyContent: isTablet ? 'flex-start' : 'space-between',
        flexDirection: isTablet ? 'column' : 'row',
        gap: isTablet ? 28 : 0,
      }}>
        <div style={{ position: 'absolute', right: -20, bottom: -40, fontFamily: fonts.display, fontStyle: 'italic', fontSize: 180, color: theme.navy, opacity: 0.06, lineHeight: 1, letterSpacing: '-0.04em', pointerEvents: 'none' }}>Repair.</div>
        <div style={{ position: 'relative', zIndex: 2, maxWidth: isTablet ? '100%' : 560 }}>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.25em', opacity: 0.7 }}>—— THE PROMISE</div>
          <div style={{ fontFamily: fonts.display, fontStyle: 'italic', fontSize: isTablet ? 38 : 52, lineHeight: 1.05, marginTop: 14 }}>
            Don't delay—<br />repair it today.
          </div>
          <div style={{ marginTop: 16, fontFamily: fonts.body, fontSize: 15, opacity: 0.85, lineHeight: 1.55 }}>
            We answer every call before it rings out. Drop in, dial in, or have us pick it up.
          </div>
        </div>
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', gap: 18, flexShrink: 0, alignSelf: isTablet ? 'flex-start' : 'center' }}>
          <button style={{
            height: 56, padding: '0 28px', border: 'none',
            background: theme.navy, color: theme.gold, borderRadius: 14,
            fontFamily: fonts.body, fontWeight: 600, fontSize: 13, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer',
          }}>Book Pickup</button>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.2em', opacity: 0.7 }}>FREE · ABOVE ₹1,000</div>
        </div>
      </div>
    </div>
  );
...
function DesktopLayout({ tweaks, setTweak }) {
  ...
  return (
    <div style={{ background: theme.bgGrad, color: theme.fg, minHeight: '100vh' }}>
      <DesktopStickyHeader
        theme={theme} fonts={fonts}
        dark={tweaks.dark} isDesktop={isDesktop}
        onToggleDark={() => setTweak('dark', !tweaks.dark)}
      />
      <main>
        <DesktopHero     theme={theme} fonts={fonts} intensity={tweaks.animation} isTablet={isTablet} />
        <DesktopSales    theme={theme} fonts={fonts} cardKind={tweaks.cardKind}   isTablet={isTablet} />
        <DesktopServices theme={theme} fonts={fonts} cardKind={tweaks.cardKind}   isTablet={isTablet} />
        <DesktopWhy      theme={theme} fonts={fonts} cardKind={tweaks.cardKind}   isTablet={isTablet} />
        <DesktopBrands   theme={theme} fonts={fonts} isTablet={isTablet} />
        <DesktopLocation theme={theme} fonts={fonts} cardKind={tweaks.cardKind}   isTablet={isTablet} />
        <DesktopContact  theme={theme} fonts={fonts} cardKind={tweaks.cardKind}   isTablet={isTablet} />
